...
function orderFactsMarkup(o){
 const f=orderFacts(o),a=f.address,sl=f.slot,links=orderLinks(a),progress=procurementProgressFacts(o);
 ...
 ${progress?`<section class="panel stack"><h3>تقدم جمع الطلب</h3><strong>${esc(progress.title)}</strong><p>${esc(progress.message)}</p>${progress.lines.map(line=>`<div class="component-row"><span>${esc(line.name)}${line.sizeLabel?' · '+esc(line.sizeLabel):''}<small>طُلب ${number(line.requested)} · جُمع ${number(line.collected)} · متبقٍ ${number(line.remaining)}</small></span></div>`).join('')}${progress.shortage?`<p>${progress.shortage.state==='pending'?'التخفيض المقترح عند الموافقة':'التخفيض المرتبط بالنقص'}: <strong>${money(progress.shortage.reduction)}</strong></p>`:''}${progress.canDecide?`<div class="stack"><p class="notice">الموافقة تعني قبول حذف الكمية غير المتوفرة ليصبح الإجمالي ${money(progress.shortage.totalIfApproved)} بعد تطبيق التخفيض المنفصل. الرفض لا يغيّر السعر ويعيد الطلب لمتابعة الجمع.</p><label>ملاحظتك على القرار<textarea id="procurement-shortage-note" minlength="3" maxlength="1000" required placeholder="اكتب سبب الموافقة أو ما تريد من الموظف متابعته"></textarea></label><div class="row wrap"><button class="btn primary" data-procurement-decision="approve_removal">الموافقة على النقص</button><button class="btn outline" data-procurement-decision="reject_removal">رفض النقص ومتابعة الجمع</button></div></div>`:progress.actionRequired?'<p class="notice">لن يتغير طلبك قبل تسجيل موافقتك الصريحة.</p>':''}<small>آخر تحديث: ${date(progress.updatedAt)}</small></section>`:''}
...
async function showOrder(id){
 const o=await get('/api/orders/'+id),f=orderFacts(o),receipt=cashReceiptFacts(o),progress=procurementProgressFacts(o);
 const d=modal('طلب '+o.number,`<div class="stack">${storeVersionMarkup(o.original_snapshot?.store_profile||o.snapshot?.store_profile)}<div class="row wrap">${badge(o.status)} ${badge(o.fulfillment_state)} ${badge(o.delivery_state)} ${badge(o.payment_state)}</div>${orderFactsMarkup(o)}${receipt?`<section class="panel stack"><h3>إيصال التحصيل النقدي</h3><p>تم تحصيل ${money(receipt.collected)} نقدًا عند الاستلام في ${date(receipt.collectedAt)}.</p>${receipt.refunded?`<p>المبلغ المُعاد: ${money(receipt.refunded)} · الصافي: ${money(receipt.net)}</p>`:''}<p class="notice">هذا إيصال تحصيل نقدي وليس فاتورة ضريبية.</p><button class="btn outline" id="download-cash-receipt">تنزيل الإيصال للطباعة أو الحفظ PDF</button></section>`:''}<section class="panel stack"><h3>موقع المندوب</h3><div id="order-tracking" role="status">اضغط التحديث لمعرفة آخر موقع مسجل.</div><button class="btn outline" id="refresh-tracking">تحديث موقع المندوب</button></section>${(o.substitutions||[]).map(s=>substitutionReview(s,o.id,true)).join('')}<section class="panel stack"><h3>الاستردادات</h3>${(o.refunds||[]).map(r=>`<div><div class="row between"><strong>${money(r.amount_halalas)}</strong>${badge(r.state)}</div><p>${esc(r.reason)}</p>${r.decision_note?`<p>${esc(r.decision_note)}</p>`:''}</div>`).join('')||'<p>لا توجد طلبات استرداد.</p>'}</section><div class="row wrap">${f.canRefund?`<button class="btn outline" data-refund-order="${esc(o.id)}">طلب استرداد مبلغ</button>`:''}${f.canCancel?`<button class="btn danger" data-cancel="${esc(o.id)}">إلغاء قبل التجهيز</button>`:''}${f.canRenewCode?`<button class="btn outline" data-code="${esc(o.id)}">تجديد رمز التسليم</button>`:''}<button class="btn outline" id="refresh-order">تحديث الطلب</button><button class="btn outline" data-support="${esc(o.id)}">الدعم</button></div></div>`);
 if(receipt)$('#download-cash-receipt',d).onclick=()=>downloadText(`jana-cash-receipt-${String(o.number||o.id).replace(/[^\p{L}\p{N}._-]+/gu,'-')}.html`,cashReceiptHtml(o),'text/html;charset=utf-8');
 if(progress?.canDecide)for(const button of $$('[data-procurement-decision]',d))button.onclick=()=>busy(button,async()=>{
  const note=$('#procurement-shortage-note',d).value.trim(),decision=button.dataset.procurementDecision;
  if([...note].length<3||[...note].length>1000)throw Error('أدخل ملاحظة واضحة من 3 إلى 1000 حرف.');
  const approved=decision==='approve_removal',message=approved?`تأكيد حذف الكمية غير المتوفرة؟ سيصبح الإجمالي ${money(progress.shortage.totalIfApproved)} بعد تطبيق التخفيض المنفصل، ولن تُضاف رسوم جديدة.`:'رفض النقص ومطالبة الموظف بمتابعة الجمع؟ لن يتغير سعر طلبك.';
  if(!confirm(message))return;
  await post('/api/orders/'+id+'/procurement-shortage-decision',{request_id:progress.shortage.requestId,expected_revision:progress.revision,decision,note});
  toast(approved?'تم تسجيل موافقتك الصريحة':'تم تسجيل الرفض واستئناف الجمع');await showOrder(id);
 }).catch(()=>{});
