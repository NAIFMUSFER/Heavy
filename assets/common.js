...
export function downloadText(name,text,type='text/plain;charset=utf-8'){const url=URL.createObjectURL(new Blob([text],{type}));const a=document.createElement('a');a.href=url;a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(url),10000);}
...
export function substitutionReview(sub,orderId='',interactive=false){
 ...
 const removal=terms.action==='remove_line',componentSwap=terms.action==='replace_component';
 const from=terms.original_component||{},to=terms.replacement_component||{};
 const safeTotals=Number.isSafeInteger(terms.total_halalas)&&Number.isSafeInteger(terms.price_difference_halalas)&&Number.isSafeInteger(terms.original_total_halalas);
 const safeComponent=componentSwap&&!!original.name&&!!replacement.name&&!!from.name&&!!to.name&&from.stock_id!==to.stock_id&&from.base_unit===to.base_unit&&Number.isSafeInteger(from.planned_base)&&Number.isSafeInteger(from.actual_base)&&from.planned_base>0&&from.actual_base>=0&&from.actual_base<from.planned_base&&from.planned_base===to.planned_base&&terms.total_halalas===terms.original_total_halalas&&terms.price_difference_halalas===0;
 const complete=!!original.name&&safeTotals&&(componentSwap?safeComponent:removal?Number.isSafeInteger(terms.subtotal_halalas)&&terms.total_halalas<=terms.original_total_halalas:!!replacement.name);
 const pending=sub.state==='pending'&&Number(sub.expires_at)>Date.now();
 const line=l=>`<strong>${esc(l.name||'تفاصيل غير متاحة')}</strong> × ${esc(l.qty??'')} ${esc(l.size_label||'')}<p>${(l.components||[]).map(c=>esc(c.name)+' — '+esc(qty({...c,base_qty:c.base_qty*l.qty}))).join('، ')}</p>${l.weight_policy?`<small>الوزن المسموح ${number(l.weight_policy.min_base)}–${number(l.weight_policy.max_base)} جرام. النقص يخفض السعر والزيادة المسموحة مجانًا.</small>`:''}`;
 const componentDetails=componentSwap?`<div><small>المكوّن الناقص</small><div><strong>${esc(from.name||'تفاصيل غير متاحة')}</strong><p>${esc(qty({...from,base_qty:from.planned_base}))} · المسجل فعليًا ${esc(qty({...from,base_qty:from.actual_base}))}</p></div></div><div><small>المكوّن البديل</small><div><strong>${esc(to.name||'تفاصيل غير متاحة')}</strong><p>${esc(qty({...to,base_qty:to.planned_base}))} من الوحدة نفسها</p></div></div><p class="notice">سعر السلة والإجمالي لن يتغيرا. لا يُحجز البديل إلا بعد موافقتك، ثم يعيد المجهز قياس جميع مكونات السلة.</p>`:'';
 return `<section class="panel stack"><div class="row between"><h3>${componentSwap?'اقتراح استبدال مكوّن في السلة':removal?'طلب حذف صنف':'اقتراح استبدال'}</h3>${badge(sub.state)}</div><div><small>${removal?'الصنف غير المتوفر':componentSwap?'السلة':'الصنف الأصلي'}</small><div>${line(original)}</div></div>${componentSwap?componentDetails:removal?'<p class="notice warning">لن يُحذف الصنف أو يتغير الإجمالي إلا بعد موافقتك الصريحة.</p>':`<div><small>البديل المقترح</small><div>${line(replacement)}</div></div>`}<div class="row between"><span>الإجمالي السابق</span><strong>${money(terms.original_total_halalas)}</strong></div><div class="row between"><span>فرق الإجمالي</span><strong>${money(terms.price_difference_halalas)}</strong></div><div class="row between"><span>الإجمالي عند الموافقة</span><strong>${money(terms.total_halalas)}</strong></div>${sub.state==='pending'?`<p class="notice">المهلة: ${date(sub.expires_at)}. ${componentSwap?'عدم الرد لا يعني الموافقة، ويبقى نقص المكوّن بحاجة إلى معالجة.':removal?'عدم الرد يبقي الصنف معلقًا ولا يعني الحذف.':'يُحجز البديل عند الموافقة إذا بقي متاحًا. عدم الرد لا يعني الموافقة.'}</p>`:''}${interactive&&pending?`<div class="row"><button class="btn primary" data-sub="${esc(sub.id)}" data-accept="true" data-order-id="${esc(orderId)}" ${complete?'':'disabled'}>${componentSwap?'أوافق على مكوّن السلة بالسعر نفسه':removal?'أوافق على حذف الصنف والإجمالي':'أوافق على البديل والإجمالي'}</button><button class="btn outline" data-sub="${esc(sub.id)}" data-accept="false" data-order-id="${esc(orderId)}">${componentSwap?'أرفض مكوّن السلة البديل':removal?'أرفض حذف الصنف':'أرفض البديل'}</button></div>`:''}${sub.state==='pending'&&!pending?'<p>انتهت المهلة. حدّث الطلب لمعرفة آخر حالة.</p>':''}</section>`;
}
