...
export function substitutionReview(sub,orderId='',interactive=false){
 ...
 const removal=terms.action==='remove_line';
 const complete=!!original.name&&Number.isSafeInteger(terms.total_halalas)&&Number.isSafeInteger(terms.price_difference_halalas)&&Number.isSafeInteger(terms.original_total_halalas)&&(!removal||Number.isSafeInteger(terms.subtotal_halalas)&&terms.total_halalas<=terms.original_total_halalas)&&(!removal&&!!replacement.name||removal);
 ...
 return `<section class="panel stack"><div class="row between"><h3>${removal?'طلب حذف صنف':'اقتراح استبدال'}</h3>${badge(sub.state)}</div><div><small>${removal?'الصنف غير المتوفر':'الصنف الأصلي'}</small><div>${line(original)}</div></div>${removal?'<p class="notice warning">لن يُحذف الصنف أو يتغير الإجمالي إلا بعد موافقتك الصريحة.</p>':`<div><small>البديل المقترح</small><div>${line(replacement)}</div></div>`}<div class="row between"><span>الإجمالي السابق</span><strong>${money(terms.original_total_halalas)}</strong></div><div class="row between"><span>فرق الإجمالي</span><strong>${money(terms.price_difference_halalas)}</strong></div><div class="row between"><span>الإجمالي عند الموافقة</span><strong>${money(terms.total_halalas)}</strong></div>${sub.state==='pending'?`<p class="notice">المهلة: ${date(sub.expires_at)}. ${removal?'عدم الرد يبقي الصنف معلقًا ولا يعني الحذف.':'يُحجز البديل عند الموافقة إذا بقي متاحًا. عدم الرد لا يعني الموافقة.'}</p>`:''}${interactive&&pending?`<div class="row"><button class="btn primary" data-sub="${esc(sub.id)}" data-accept="true" data-order-id="${esc(orderId)}" ${complete?'':'disabled'}>${removal?'أوافق على حذف الصنف والإجمالي':'أوافق على البديل والإجمالي'}</button><button class="btn outline" data-sub="${esc(sub.id)}" data-accept="false" data-order-id="${esc(orderId)}">${removal?'أرفض حذف الصنف':'أرفض البديل'}</button></div>`:''}${sub.state==='pending'&&!pending?'<p>انتهت المهلة. حدّث الطلب لمعرفة آخر حالة.</p>':''}</section>`;
}
