...
export async function deliverNext({repository,provider,now=Date.now}){
 const job=await repository.claim('email','resend');if(!job)return {state:'idle'};
 let result;
 try{
  if(job.channel!=='email'||job.provider!=='resend'||now()>=job.retry_until)throw new ProviderError('DELIVERY_RECONCILIATION_REQUIRED');
...
export function configuredWorker({env=process.env,fetchImpl=fetch,now=Date.now}={}){
 ...
 return ()=>deliverNext({repository,provider,now});
}
