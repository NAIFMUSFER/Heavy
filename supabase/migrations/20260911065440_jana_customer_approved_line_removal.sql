-- Removing an unavailable order line is a customer decision, never a picker shortcut.
-- The proposal freezes the exact lower total; inventory changes only after acceptance.
ALTER TABLE public.picking_line_issues DROP CONSTRAINT picking_line_issues_state_check;
ALTER TABLE public.picking_line_issues ADD CONSTRAINT picking_line_issues_state_check
 CHECK(state IN ('open','replaced','restored','removed'));

CREATE FUNCTION public.jana_propose_line_removal(p_token text,p_order_id text,p_line_id text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,extensions,pg_temp AS $$
DECLARE u public.users;o public.orders;target jsonb;terms jsonb;sid text;subtotal bigint;discount bigint;total bigint;nowms bigint;
BEGIN
 ...
 IF jsonb_array_length(o.snapshot::jsonb->'lines')<=1 THEN RAISE EXCEPTION 'cannot_remove_last_line';END IF;
 ...
 subtotal=(o.snapshot->>'subtotal_halalas')::bigint-(target->>'line_total_halalas')::bigint;
 IF subtotal<0 THEN RAISE EXCEPTION 'line_removal_invalid';END IF;
 ...
 terms=jsonb_build_object('action','remove_line','original_line',target,'replacement_line',null,
 ...
 PERFORM public.jana_picking_issue(p_token,o.id,p_line_id,'بانتظار موافقة العميل على حذف الصنف غير المتوفر',false);
 ...
 VALUES(sid,o.id,p_line_id,target->>'offering_id',terms,'hold_for_resolution','pending',nowms+900000,u.id,nowms);
 ...
 VALUES('ntf-'||replace(gen_random_uuid()::text,'-',''),o.user_id,'remove-'||sid,'حذف صنف يحتاج موافقتك','الصنف غير متوفر. راجع الصنف والإجمالي الأقل قبل الموافقة؛ عدم الرد لا يعني الموافقة.',o.id,false,nowms);
 INSERT INTO public.order_events(id,order_id,actor_id,event,reason,states,created_at)
 VALUES('evt-'||replace(gen_random_uuid()::text,'-',''),o.id,u.id,'line_removal_proposed',p_line_id,jsonb_build_object('substitution_id',sid,'price_difference_halalas',total-o.total_halalas),nowms);
 INSERT INTO public.audit_log(id,actor_id,action,entity_id,detail,created_at)
 VALUES('aud-'||replace(gen_random_uuid()::text,'-',''),u.id,'line_removal_proposed',sid,jsonb_build_object('role',u.role,'order_id',o.id,'line',target,'total_before_halalas',o.total_halalas,'total_after_halalas',total),nowms);
...
BEGIN
 IF p_reason_kind IS NULL OR p_reason_kind NOT IN ('substitution','weight','line_removal') THEN RAISE EXCEPTION 'invalid_operation';END IF;
 SELECT * INTO o FROM public.orders WHERE id=p_order_id FOR UPDATE;
...
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,extensions,pg_temp AS $$
DECLARE u public.users;s public.substitutions;o public.orders;oid text;newlines jsonb;allocations jsonb;result jsonb;nowms bigint;removal boolean;
BEGIN
 ...
 removal=coalesce(s.proposed->>'action','')='remove_line';
 nowms=(extract(epoch from clock_timestamp())*1000)::bigint;
 ...
 ELSE
  IF p_accept THEN
   ...
   IF removal THEN
    SELECT coalesce(jsonb_agg(l ORDER BY n) FILTER(WHERE l->>'line_id'<>s.line_id),'[]'::jsonb) INTO newlines
    FROM jsonb_array_elements(o.snapshot::jsonb->'lines') WITH ORDINALITY e(l,n);
   ...
   UPDATE public.picking_line_issues SET state=CASE WHEN removal THEN 'removed' ELSE 'replaced' END,updated_at=nowms WHERE order_id=o.id AND line_id=s.line_id;
  END IF;
  result=jsonb_build_object('id',s.id,'order_id',o.id,'action',CASE WHEN removal THEN 'remove_line' ELSE 'replace_line' END,'state',CASE WHEN p_accept THEN 'accepted' ELSE 'rejected' END,'total_halalas',(SELECT total_halalas FROM public.orders WHERE id=o.id));
 ...
 VALUES('evt-'||replace(gen_random_uuid()::text,'-',''),o.id,u.id,
  CASE WHEN removal AND s.expires_at<=nowms THEN 'line_removal_expired' WHEN removal AND p_accept THEN 'line_removal_accepted' WHEN removal THEN 'line_removal_rejected' WHEN s.expires_at<=nowms THEN 'substitution_expired' WHEN p_accept THEN 'substitution_accepted' ELSE 'substitution_rejected' END,s.id,result,nowms);
 INSERT INTO public.audit_log(id,actor_id,action,entity_id,detail,created_at)
 VALUES('aud-'||replace(gen_random_uuid()::text,'-',''),u.id,CASE WHEN removal THEN 'line_removal_decision' ELSE 'substitution_decision' END,s.id,jsonb_build_object('role',u.role,'before','pending','after',result),nowms);
 RETURN result;
END$$;
...
DECLARE o public.orders;s public.substitutions;nowms bigint:=(extract(epoch from clock_timestamp())*1000)::bigint;n int:=0;removal boolean;
BEGIN
 FOR o IN SELECT * FROM public.orders WHERE id IN(SELECT order_id FROM public.substitutions WHERE state='pending' AND expires_at<=nowms) ORDER BY id FOR UPDATE SKIP LOCKED LOOP
  FOR s IN SELECT * FROM public.substitutions WHERE order_id=o.id AND state='pending' AND expires_at<=nowms FOR UPDATE LOOP
   ...
   INSERT INTO public.notifications(id,user_id,dedupe_key,title,body,order_id,is_read,created_at)
   VALUES('ntf-'||replace(gen_random_uuid()::text,'-',''),o.user_id,'sub-expired-'||s.id,CASE WHEN removal THEN 'انتهت مهلة حذف الصنف' ELSE 'انتهت مهلة البديل' END,
    CASE WHEN removal THEN 'لم يُحذف الصنف. يظل بحاجة إلى معالجة من فريق التجهيز.' ELSE 'لم يُعتمد البديل. يظل الصنف بحاجة إلى معالجة من فريق التجهيز.' END,o.id,false,nowms);
   INSERT INTO public.order_events(id,order_id,actor_id,event,reason,states,created_at)
   VALUES('evt-'||replace(gen_random_uuid()::text,'-',''),o.id,null,CASE WHEN removal THEN 'line_removal_expired' ELSE 'substitution_expired' END,s.id,jsonb_build_object('line_id',s.line_id,'requires_resolution',true),nowms);n=n+1;
  END LOOP;
 END LOOP;
 RETURN n;
END$$;
...
BEGIN
 u=public.jana_auth_user(p_token);
 IF p_operation NOT IN ('substitution.propose','substitution.decide','line.removal.propose','line.unavailable','line.restore','line.actual','picking.finish') OR p_operation IS NULL THEN RAISE EXCEPTION 'invalid_operation';END IF;
...
BEGIN
 ...
 WITH events AS MATERIALIZED (
  ...
  AND event IN ('order_created','picking','start_picking','ready','out_for_delivery','delivery_failed','delivered','cancelled','substitution_proposed','substitution_accepted','substitution_rejected','line_removal_proposed','line_removal_accepted','line_removal_rejected','line_removal_expired','refund_completed')
...
REVOKE ALL ON FUNCTION public.jana_propose_line_removal(text,text,text),public.jana_decide_substitution(text,text,boolean),public.jana_expire_substitutions(),public.jana_picking_write(text,text,text,jsonb),public.jana_order_detail(text,text) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.jana_propose_line_removal(text,text,text),public.jana_decide_substitution(text,text,boolean),public.jana_expire_substitutions(),public.jana_picking_write(text,text,text,jsonb),public.jana_order_detail(text,text) TO service_role;
