-- Add transactional replay for externally retried critical writes.
-- The called operations retain their own row locks and business authorization.
...
BEGIN
 ...
 ELSIF p_operation IN ('cod.settle','refund.create') THEN
 ...
 CASE p_operation
  ...
  WHEN 'cod.settle' THEN result=public.jana_finance_settle(p_token,p_payload->>'order_id',p_payload->>'reference');
  WHEN 'refund.create' THEN result=public.jana_admin_refund(p_token,p_payload->>'order_id',(p_payload->>'amount_halalas')::bigint,p_payload->>'reason');
...
BEGIN
 BEGIN
  ...
  -- Dedicated second order for persisted financial retries. Fixtures still roll back.
  q=public.jana_create_quote(t,p||'s',p||'addr',jsonb_build_array(jsonb_build_object('offering_id',p||'off','qty',1)));
  o=public.jana_critical_write(t,p||'confirm','order.confirm',jsonb_build_object('quote_id',q->>'id'));
  r=public.jana_critical_write(t,p||'confirm','order.confirm',jsonb_build_object('quote_id',q->>'id'));
  SELECT count(*) INTO n FROM public.orders WHERE quote_id=q->>'id';
  results=results||jsonb_build_array(jsonb_build_object('test','durable_confirmation_returns_original','pass',r=o AND n=1));
  BEGIN PERFORM public.jana_critical_write(t,p||'confirm','order.confirm',jsonb_build_object('quote_id','different'));ok=false;
  EXCEPTION WHEN OTHERS THEN ok=SQLERRM='idempotency_conflict'; END;
  results=results||jsonb_build_array(jsonb_build_object('test','critical_key_rejects_changed_body','pass',ok));
  BEGIN PERFORM public.jana_critical_write(t,p||'unauth','cod.settle',jsonb_build_object('order_id',o->>'id','reference','fixture-ref'));ok=false;
  EXCEPTION WHEN OTHERS THEN ok=SQLERRM='forbidden'; END;
  results=results||jsonb_build_array(jsonb_build_object('test','customer_cannot_use_financial_dispatch','pass',ok));
  UPDATE public.orders SET fulfillment_state='ready',delivery_state='out_for_delivery',courier_id=p||'c',code_hash=encode(extensions.digest('123456','sha256'),'hex'),code_expires_at=nowms+60000 WHERE id=o->>'id';
  r=public.jana_critical_write(ct,p||'deliver-bad','order.deliver',jsonb_build_object('order_id',o->>'id','code','000000'));
  SELECT code_attempts INTO n FROM public.orders WHERE id=o->>'id';
  SELECT count(*) INTO before_n FROM public.idempotency_records WHERE key=p||'deliver-bad';
  results=results||jsonb_build_array(jsonb_build_object('test','failed_otp_updates_counter_without_success_cache','pass',r->>'_error'='invalid_delivery_code' AND n=1 AND before_n=0));
  r=public.jana_critical_write(ct,p||'deliver-good','order.deliver',jsonb_build_object('order_id',o->>'id','code','123456'));
  q2=public.jana_critical_write(ct,p||'deliver-good','order.deliver',jsonb_build_object('order_id',o->>'id','code','123456'));
  results=results||jsonb_build_array(jsonb_build_object('test','delivery_retry_returns_original','pass',r=q2 AND (r->>'collected_halalas')::bigint=0));
  r=public.jana_critical_write(ct,p||'collect','cod.collect',jsonb_build_object('order_id',o->>'id','amount_halalas',2000));
  ...
  q2=public.jana_critical_write(atok,p||'settle','cod.settle',jsonb_build_object('order_id',o->>'id','reference','Audit deposit'));
  SELECT count(*) INTO n FROM public.audit_log WHERE entity_id=o->>'id' AND action='cash_settled';
  results=results||jsonb_build_array(jsonb_build_object('test','settlement_retry_posts_once','pass',r=q2 AND n=1 AND (r->>'settled_halalas')::bigint=1900));
...
DO $gate$ BEGIN IF EXISTS(SELECT 1 FROM jsonb_array_elements(current_setting('jana.audit_results')::jsonb) e WHERE e->>'pass'<>'true') THEN RAISE EXCEPTION 'JANA critical-write regression gate failed: %',current_setting('jana.audit_results'); END IF; END $gate$;
