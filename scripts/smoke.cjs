...
async function smoke(){
 ...
 const login=await get('/api/auth/login',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email:'smoke-invalid@example.invalid',password:null})});assert.equal(login.status,401);const cookieNames=login.headers.getSetCookie().map(x=>x.split('=')[0]);assert.equal(cookieNames.filter(x=>/^jana_(session|csrf)$/.test(x)).length,0,'failed login must not issue JANA authentication cookies');
