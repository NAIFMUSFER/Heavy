...
async function smoke(){
 ...
 for(const p of ['/','/admin.html','/picker.html','/courier.html','/assets/shop.js','/assets/ops.js','/assets/common.js','/assets/zone-map.js','/assets/styles.css','/sw.js']){const r=await get(p);assert.equal(r.status,200,p);assert.ok((await r.text()).length>100,p);assert.ok(r.headers.get('content-security-policy'),p)}
 ...
 console.log(JSON.stringify({ok:true,commit:version.commit,checks:21,base}));
}
