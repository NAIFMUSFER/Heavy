...
try {
 ...
 for(const [prefix,count] of [['shop',5],['ops',4],['common',1],['zone-map',1]]){
  const files=['common','zone-map'].includes(prefix)?['assets/'+prefix+'.js']:Array.from({length:count},(_,i)=>`assets/${prefix}.part${String(i+1).padStart(2,'0')}.js`);
