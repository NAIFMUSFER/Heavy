...
try {
 for(const f of ['server.js','server/notification-worker.mjs','server/providers/notification-repository.mjs','scripts/notification-worker.mjs'])execFileSync(process.execPath,['--check',f],{stdio:'inherit'});
 for(const [prefix,count] of [['shop',5],['ops',4],['common',1],['zone-map',1],['address',1],['order',1],['checkout',1]]){
  const files=['common','zone-map','address','order','checkout'].includes(prefix)?['assets/'+prefix+'.js']:Array.from({length:count},(_,i)=>`assets/${prefix}.part${String(i+1).padStart(2,'0')}.js`);
