...
test('procurement finance writes are strict, path-bound and preserve explicit evidence',async()=>{
 const jobId='prc-'+'9'.repeat(32),purchaseId='pur-'+'1'.repeat(32),fundingId='pfd-'+'2'.repeat(32),token=bearer.authorization.slice(7),headers={...bearer,'idempotency-key':'finance-procurement-fixture'};
 calls=[];response={id:fundingId,job_id:jobId};
 let r=await handlers['jana-api'](request('jana-api',`/api/ops/procurement/${jobId}/funding`,{method:'POST',headers,body:JSON.stringify({purchase_record_id:`  ${purchaseId}  `,funding_source:'employee_paid',evidence_reference:'  ADV-100  ',note:'  Employee paid supplier  '})}));
 assert.equal(r.status,201);assert.equal(calls.length,1);assert.ok(calls[0].url.endsWith('/jana_ops_procurement_funding_record'));assert.deepEqual(calls[0].body,{p_token:token,p_idem_key:'finance-procurement-fixture',p_job_id:jobId,p_purchase_record_id:purchaseId,p_funding_source:'employee_paid',p_evidence_reference:'ADV-100',p_note:'Employee paid supplier'});
 calls=[];response={id:'pst-fixture',job_id:jobId};
 r=await handlers['jana-api'](request('jana-api',`/api/ops/procurement/${jobId}/settlements`,{method:'POST',headers,body:JSON.stringify({funding_id:fundingId,amount_halalas:1250,payment_reference:'  PAY-100  ',note:'  Partial actual payment  '})}));
 assert.equal(r.status,201);assert.equal(calls.length,1);assert.ok(calls[0].url.endsWith('/jana_ops_procurement_settlement_record'));assert.deepEqual(calls[0].body,{p_token:token,p_idem_key:'finance-procurement-fixture',p_job_id:jobId,p_funding_id:fundingId,p_amount_halalas:1250,p_payment_reference:'PAY-100',p_note:'Partial actual payment'});
 for(const [path,bad] of [['funding',{purchase_record_id:purchaseId,funding_source:'automatic',evidence_reference:'ADV-100',note:'Valid note'}],['funding',{purchase_record_id:purchaseId,funding_source:'company_paid',evidence_reference:'x',note:'Valid note'}],['settlements',{funding_id:fundingId,amount_halalas:1.5,payment_reference:'PAY-100',note:'Valid note'}],['settlements',{funding_id:'bad',amount_halalas:100,payment_reference:'PAY-100',note:'Valid note'}]]){calls=[];r=await handlers['jana-api'](request('jana-api',`/api/ops/procurement/${jobId}/${path}`,{method:'POST',headers,body:JSON.stringify(bad)}));assert.equal(r.status,422);assert.equal(calls.length,0)}
});
test('procurement handover is strict path-bound and keeps preparation separate from courier acceptance',async()=>{
 const jobId='prc-'+'7'.repeat(32),requestId='phr-'+'6'.repeat(32),token=bearer.authorization.slice(7),headers={...bearer,'idempotency-key':'handover-fixture'};
 calls=[];response={id:'fixture'};let r=await handlers['jana-api'](request('jana-api','/api/ops/procurement-couriers',{headers:bearer}));assert.equal(r.status,200);assert.ok(calls[0].url.endsWith('/jana_procurement_active_couriers'));assert.deepEqual(calls[0].body,{p_token:token});
 calls=[];r=await handlers['jana-api'](request('jana-api',`/api/ops/procurement/${jobId}/handover`,{method:'POST',headers,body:JSON.stringify({courier_id:'  courier-fixture  ',expected_revision:7,note:'  Physical custody prepared  '})}));
 assert.equal(r.status,201);assert.equal(calls.length,1);assert.ok(calls[0].url.endsWith('/jana_ops_procurement_handover_prepare'));assert.deepEqual(calls[0].body,{p_token:token,p_idem_key:'handover-fixture',p_job_id:jobId,p_courier_id:'courier-fixture',p_expected_revision:7,p_note:'Physical custody prepared'});
 calls=[];r=await handlers['jana-api'](request('jana-api',`/api/ops/procurement/${jobId}/handover-accept`,{method:'POST',headers,body:JSON.stringify({request_id:`  ${requestId}  `,expected_revision:8,note:'  Counted and accepted  '})}));
 assert.equal(r.status,201);assert.equal(calls.length,1);assert.ok(calls[0].url.endsWith('/jana_ops_procurement_handover_accept'));assert.deepEqual(calls[0].body,{p_token:token,p_idem_key:'handover-fixture',p_job_id:jobId,p_request_id:requestId,p_expected_revision:8,p_note:'Counted and accepted'});
 for(const [path,bad] of [['handover',{courier_id:'courier-fixture',expected_revision:0,note:'Valid note'}],['handover',{courier_id:'courier-fixture',expected_revision:7,note:'x'}],['handover-accept',{request_id:'bad',expected_revision:8,note:'Valid note'}],['handover-accept',{request_id:requestId,expected_revision:'8',note:'Valid note'}]]){calls=[];r=await handlers['jana-api'](request('jana-api',`/api/ops/procurement/${jobId}/${path}`,{method:'POST',headers,body:JSON.stringify(bad)}));assert.equal(r.status,422);assert.equal(calls.length,0)}
});
test('procurement handover conflicts remain actionable at the Edge boundary',async()=>{
 const jobId='prc-'+'6'.repeat(32),prepare={courier_id:'courier-fixture',expected_revision:7,note:'Physical custody prepared'},accept={request_id:'phr-'+'5'.repeat(32),expected_revision:8,note:'Counted and accepted'};
 for(const [path,body,message,status,code] of [['handover',prepare,'procurement_funding_required',409,'PROCUREMENT_FUNDING_REQUIRED'],['handover',prepare,'procurement_courier_invalid',409,'PROCUREMENT_COURIER'],['handover',prepare,'procurement_handover_exists',409,'PROCUREMENT_HANDOVER_EXISTS'],['handover-accept',accept,'procurement_handover_not_found',404,'PROCUREMENT_HANDOVER_NOT_FOUND'],['handover-accept',accept,'procurement_handover_evidence_changed',409,'PROCUREMENT_HANDOVER_EVIDENCE']]){calls=[];response={_error:message,status};const r=await handlers['jana-api'](request('jana-api',`/api/ops/procurement/${jobId}/${path}`,{method:'POST',headers:{...bearer,'idempotency-key':'handover-error-fixture'},body:JSON.stringify(body)}));assert.equal(r.status,status);assert.equal((await r.json()).error.code,code)}
});
test('procurement finance conflicts stay actionable without masking settlement state',async()=>{
 const jobId='prc-'+'8'.repeat(32),funding={purchase_record_id:'pur-'+'3'.repeat(32),funding_source:'supplier_credit',evidence_reference:'CREDIT-1',note:'Actual supplier credit'},settlement={funding_id:'pfd-'+'4'.repeat(32),amount_halalas:100,payment_reference:'PAY-1',note:'Actual payment'};
 for(const [path,body,message,status,code] of [['funding',funding,'procurement_funding_exists',409,'PROCUREMENT_FUNDING_EXISTS'],['funding',funding,'procurement_funding_not_found',404,'PROCUREMENT_FUNDING_NOT_FOUND'],['funding',funding,'procurement_funding_state_invalid',409,'PROCUREMENT_FUNDING_STATE'],['settlements',settlement,'procurement_settlement_not_payable',409,'PROCUREMENT_SETTLEMENT_NOT_PAYABLE'],['settlements',settlement,'procurement_settlement_exceeded',409,'PROCUREMENT_SETTLEMENT_EXCEEDED'],['settlements',settlement,'procurement_settlement_not_found',404,'PROCUREMENT_SETTLEMENT_NOT_FOUND']]){calls=[];response={_error:message,status};const r=await handlers['jana-api'](request('jana-api',`/api/ops/procurement/${jobId}/${path}`,{method:'POST',headers:{...bearer,'idempotency-key':'procurement-finance-error'},body:JSON.stringify(body)}));assert.equal(r.status,status);assert.equal((await r.json()).error.code,code)}
 response={ok:true};
});
