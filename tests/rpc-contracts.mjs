...
const procurementCases=[
 ['POST','/api/ops/procurement/prc-11111111111111111111111111111111/assignment',{employee_id:'fixture-picker',expected_revision:1,reason:'Fixture procurement assignment'}],
 ['POST','/api/ops/procurement/prc-11111111111111111111111111111111/purchases',{expected_revision:1,supplier_id:'fixture-supplier',pickup_site_id:'pup-11111111111111111111111111111111',document_reference:'Fixture receipt',note:'Fixture supplier visit',lines:[{line_id:'fixture-line',collected_qty:1.25,actual_cost_halalas:100,quality_note:'Fixture quality'}]}],
 ['POST','/api/ops/procurement/prc-11111111111111111111111111111111/funding',{purchase_record_id:'pur-11111111111111111111111111111111',funding_source:'company_paid',evidence_reference:'Fixture funding',note:'Fixture explicit funding'}],
 ['POST','/api/ops/procurement/prc-11111111111111111111111111111111/settlements',{funding_id:'pfd-11111111111111111111111111111111',amount_halalas:100,payment_reference:'Fixture payment',note:'Fixture actual settlement'}],
 ['POST','/api/ops/procurement/prc-11111111111111111111111111111111/handover',{courier_id:'fixture-courier',expected_revision:7,note:'Fixture custody preparation'}],
 ['POST','/api/ops/procurement/prc-11111111111111111111111111111111/handover-accept',{request_id:'phr-11111111111111111111111111111111',expected_revision:8,note:'Fixture custody acceptance'}]
];
for(const [method,path,body] of procurementCases){const before=calls;const r=await handler(new Request('https://edge.example/jana-api'+path,{method,headers:{authorization:'Bearer fixture-session','content-type':'application/json','idempotency-key':'procurement-contract-key','x-csrf-token':'fixture-csrf'},body:JSON.stringify(body)}));assert.ok(r.ok,`${path}: ${r.status}`);assert.equal(calls,before+1)}
console.log(JSON.stringify({passed:procurementCases.length,contract_source:'restored PostgreSQL pg_proc',scope:'path-bound procurement assignment and purchase RPC signatures'}));
