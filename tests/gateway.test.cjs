...
test('customer and operations bundles remain valid and protected',async t=>{const f=await fixture(t);for(const p of ['/','/admin.html','/picker.html','/courier.html','/assets/shop.js','/assets/ops.js','/assets/styles.css','/assets/common.js','/assets/zone-map.js','/assets/address.js','/assets/checkout.js']){const r=await fetch(f.base+p);assert.equal(r.status,200);assert.ok((await r.text()).length>100);assert.match(r.headers.get('content-security-policy'),/script-src 'self';/);assert.equal(r.headers.get('x-frame-options'),'DENY')}});
