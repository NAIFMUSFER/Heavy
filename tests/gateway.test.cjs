...
test('customer, status and operations bundles remain valid and protected',async t=>{const f=await fixture(t);for(const p of ['/','/status.html','/admin.html','/picker.html','/courier.html','/assets/shop.js','/assets/status.js','/assets/ops.js','/assets/styles.css','/assets/common.js','/assets/catalog-import.js','/assets/stock-import.js','/assets/ops-exports.js','/assets/zone-map.js','/assets/address.js','/assets/checkout.js','/assets/local-cart.js','/assets/input.js']){const r=await fetch(f.base+p);assert.equal(r.status,200);assert.ok((await r.text()).length>100);assert.match(r.headers.get('content-security-policy'),/script-src 'self';/);assert.equal(r.headers.get('x-frame-options'),'DENY')}});
...
test('owner handoff links are served without a database and retain real workspace destinations',async t=>{
 ...
 for(const href of ['/admin.html#launch','/status.html','/admin.html#pickup-sites','/picker.html#procurement','/courier.html#orders','/admin.html#finance','/admin.html#support']){assert.ok(html.includes('href="'+href+'"'));assert.equal((await fetch(f.base+href.split('#')[0],{method:'HEAD'})).status,200)}
...
test('readiness preserves every dependency result when one check throws',async t=>{const f=await fixture(t,async u=>{if(u.includes('jana-critical'))throw Error('fixture offline');return Response.json({ok:true})});const r=await fetch(f.base+'/ready'),body=await r.json();assert.equal(r.status,503);assert.equal(f.calls.length,3);assert.deepEqual(body.dependencies.map(x=>[x.name,x.ok]),[['jana-api',true],['jana-critical',false],['jana-ops-extra',true]])});
...
test('safe headers and body reach correct fixed upstream',async t=>{const f=await fixture(t);for(const [p,name] of [['/api/quotes','jana-critical'],['/api/ops/lots','jana-ops-extra'],['/api/ops/products','jana-ops-extra'],['/api/ops/products/import','jana-ops-extra'],['/api/ops/product-versions/fixture/activate','jana-ops-extra'],['/api/ops/procurement/prc-11111111111111111111111111111111/assignment','jana-api'],['/api/ops/procurement/prc-11111111111111111111111111111111/purchases','jana-api'],['/api/ops/procurement/prc-11111111111111111111111111111111/funding','jana-api'],['/api/ops/procurement/prc-11111111111111111111111111111111/settlements','jana-api'],['/api/orders','jana-api']]){const r=await fetch(f.base+p,{method:'POST',headers:{origin:'https://jana-fresh-app.onrender.com',authorization:'Bearer fixture','idempotency-key':'fixture-key','x-csrf-token':'csrf','x-nf-request-id':'spoof','content-type':'application/json'},body:'{}'});assert.equal(r.status,200);const [url,init]=f.calls.at(-1);assert.ok(url.includes('/'+name+p));assert.equal(init.headers['idempotency-key'],'fixture-key');assert.equal(init.headers['x-nf-request-id'],undefined);assert.equal(init.redirect,'manual');assert.ok(init.signal)}});
test('courier custody routes stay on jana-api with retry headers intact',async t=>{const f=await fixture(t);for(const suffix of ['handover','handover-accept']){const p='/api/ops/procurement/prc-11111111111111111111111111111111/'+suffix,r=await fetch(f.base+p,{method:'POST',headers:{origin:'https://jana-fresh-app.onrender.com',authorization:'Bearer fixture','idempotency-key':'custody-key','content-type':'application/json'},body:'{}'});assert.equal(r.status,200);const [url,init]=f.calls.at(-1);assert.ok(url.includes('/jana-api'+p));assert.equal(init.headers['idempotency-key'],'custody-key')}});
