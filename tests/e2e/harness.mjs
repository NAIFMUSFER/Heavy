...
export async function startHarness(){
 ...
 return{base:'http://127.0.0.1:'+server.address().port,failures,setEdgeDelay:milliseconds=>{assert.ok(Number.isInteger(milliseconds)&&milliseconds>=0&&milliseconds<=2000);control.edgeDelayMs=milliseconds},close:async()=>{await new Promise(resolve=>{server.close(resolve);server.closeAllConnections()});globalThis.fetch=nativeFetch}};
}
