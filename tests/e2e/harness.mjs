...
export async function startHarness(){
 ...
 for(const name of ['jana-api','jana-critical','jana-ops-extra']){await import('../../supabase/functions/'+name+'/index.ts');const handler=capture;handlers[name]=request=>requestScope.run({legacyAdmission:new URL(request.url).searchParams.get('__jana_disposable_legacy_admission')==='1'},()=>handler(request))}
...
export async function attachBrowser(context,base,control={}){
 ...
 await context.route('**/*',async route=>{const u=new URL(route.request().url());if(u.origin!=='https://jana-fresh-app.onrender.com')return route.abort('blockedbyclient');const target=new URL(base+u.pathname+u.search);if(control.legacyAdmission)target.searchParams.set('__jana_disposable_legacy_admission','1');const response=await route.fetch({url:target.href,maxRedirects:0,timeout:20000});const decision=await control.beforeResponse?.(u,response,route.request());if(decision==='disconnect')return route.abort('connectionreset');await route.fulfill({response})});
}
