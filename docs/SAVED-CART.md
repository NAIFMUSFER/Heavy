...
Device persistence uses the shared validated cart controller. A malformed saved record is preserved and exposes retry or an explicit local reset; it cannot crash storefront rendering. Writes are serialized, and visible selections change only after storage succeeds. Resetting this device's cart does not delete the account-saved copy or cancel orders. Account restoration also awaits local persistence before reporting success. This does not provide automatic merging between browser tabs; see [REVIEW-2026-09-11.md](REVIEW-2026-09-11.md) for verification scope.
