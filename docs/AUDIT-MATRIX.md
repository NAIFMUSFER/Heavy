...
Latest verified web release: **0faf8656eecd47924c409414c7717a6d178dd4f5**, LIVE **11 September 2026 at 12:26:43 UTC** via Render **dep-dahv6pijnfac73a9bek0**. Exact-commit [verification and public smoke](https://github.com/NAIFMUSFER/Heavy/actions/runs/34598305741) passed **27 production checks** on retry after the automatic Render trigger did not start a deployment. This release passed **313 Node tests**, **76 browser checks**, the full PostgreSQL gate, populated recovery and isolated capacity. Supabase is at **90 migrations**, with ACTIVE **jana-api v32**, jana-critical v6 and jana-ops-extra v23; custom-session authentication and intentional `verify_jwt:false` are unchanged. The owner can now validate and atomically import up to 50 products / 500 sellable sizes as inactive drafts from the catalog page. All 47 monitored commercial/configuration table counts and hashes matched before and after schema/Edge application; post-release deep health is green. Older dated checkpoints below retain historical states.
