...
| Database | 58 migrations applied | 34-check PostgreSQL regression gate passed; no fixture users retained | Schema replay and sixteen-client races passed; production-data backup restore outstanding | Verify isolated data restore |
| Edge | 51 local transport tests pass | CSRF, JSON/body limits, password errors, quote idempotency, Origin enforcement | API v12, critical v5, operations v9 active | Keep function grants restricted |
...
| Customer web | Existing RTL store/PWA | Verified JS bundles, addresses, checkout, orders, favorites, lists | End-to-end behavior, new address/coupon UI native builds passed; Render merge pending | Test browser and complete missing workflows |
