...
| Database | 59 migrations applied | 34-check PostgreSQL regression gate passed; no fixture users retained | Schema replay and sixteen-client races passed; production-data backup restore outstanding | Verify isolated data restore |
| Edge | 52 local transport tests pass | CSRF, JSON/body limits, password errors, quote idempotency, Origin enforcement | API v12, critical v5, operations v10 active | Keep function grants restricted |
...
| Customer web | Existing RTL store/PWA | Verified JS bundles, addresses, checkout, orders, favorites, lists | End-to-end behavior, new address/coupon UI native builds passed; Render address/coupon merge live; cost reporting UI pending | Test browser and complete missing workflows |
...
| Database security | JANA application tables/RPCs restricted | 34 RLS app tables; zero client execute on JANA RPCs | PostGIS owner grants still visible; prior REVOKE did not remove them | Supported Supabase administrator fix, not RLS on extension objects |
...
10 September checkpoint: cost migration `20260910021130` applied after successful disposable-database CI (`34402318961`). Cost ledger is append-only with no direct client grants; profit is unknown when consumed purchase cost is missing. Current reporting UI changes still await release.
