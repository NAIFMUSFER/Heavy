...
| Gateway | Security repairs tested locally | 15 HTTP tests; source allowlist; fixed upstream; 64 KiB request cap; timeouts; CSP; separate cookies | Deployment of current source pending | Verify /version equals deployed commit and run smoke |
| Database | 52 migrations applied | Previous 26-check PostgreSQL regression gate passed; no fixture users retained | Historical restore and multi-connection concurrency unverified | Recover full reproducible history and run isolated restore/concurrency |
| Edge | 27 local transport tests pass | CSRF, JSON/body limits, password errors, quote idempotency, Origin enforcement | Current origin patch deployment pending | Deploy 3 functions and verify versions |
...
| Inventory | Receipt/inspection/reservation/consumption RPCs exist | Acceptance once, rejected lot exclusion, bounds and cancellation regression checks | Inventory role lacks direct UI entry; cost completeness and cycle count not verified | Provide role entry and inspect cost/locking |
...
| COD and refunds | Delivery and cash separate; settlement reference required | Database regression gate | Generic finance idempotency, partial settlement and refund cash source need review | Complete ledger and retry semantics |
...
| Android baseline | Debug native build succeeded | Actions run 34386919435 at df78003 | Current revision pending; Play signing absent | Native CI and owner Play credentials for distribution |
| iOS baseline | Simulator compile succeeded | Actions run 34389041580 at 2291c2d | Current revision pending; distribution signing absent | Native CI and owner Apple credentials for distribution |
| CI | Prior mobile and smoke gates verified | GitHub runs inspected; latest iOS predecessor failed but corrected run passed | Old smoke did not bind to commit | New smoke requires exact /version SHA; do not call queued deployment live |
