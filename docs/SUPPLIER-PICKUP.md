...
2. **Complete assigned procurement work.** Job creation, assignment, immutable multi-supplier purchase evidence, funding classification, bounded role-scoped staff reads, read-only web/native staff workspaces and read-only customer collection progress are implemented. Build role-scoped transaction controls only after the write and exception boundaries are complete. Keep each order's collected goods separate and preserve historical pickup snapshots.
