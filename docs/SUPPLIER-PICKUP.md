...
2. **Complete assigned procurement work.** Job creation, assignment, immutable multi-supplier purchase evidence, funding classification, bounded role-scoped reads and a read-only web workspace are implemented. Build transaction controls, the native workspace and customer-visible progress only after the write and exception boundaries are complete. Keep each order's collected goods separate and preserve historical pickup snapshots.
