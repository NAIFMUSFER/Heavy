...
2. **Complete assigned procurement work.** Job creation, assignment, immutable multi-supplier purchase evidence and funding classification are implemented. Add bounded staff reads and customer-visible progress only after exceptions are complete. Keep each order's collected goods separate and preserve historical pickup snapshots.
