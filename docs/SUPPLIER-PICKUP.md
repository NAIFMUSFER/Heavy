...
2. **Complete assigned procurement work.** Job creation, assignment, immutable multi-supplier purchase evidence, funding classification, bounded role-scoped reads and read-only web/native workspaces are implemented. Build transaction controls and customer-visible progress only after the write and exception boundaries are complete. Keep each order's collected goods separate and preserve historical pickup snapshots.
