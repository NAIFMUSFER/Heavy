...
## Phase 10 candidate: read-only staff web workspace
...
- Local `npm test` passes 359/359 checks. Review CI and publication are recorded separately in [RELEASE-EVIDENCE.md](RELEASE-EVIDENCE.md); this section does not claim deployment.
...
2. **Complete assigned procurement work.** Job creation, assignment, immutable multi-supplier purchase evidence, funding classification, bounded role-scoped reads and a read-only web workspace are implemented/candidate as stated above. Build transaction controls, the native workspace and customer-visible progress only after the write and exception boundaries are complete. Keep each order's collected goods separate and preserve historical pickup snapshots.
