...
2. **Complete assigned procurement work.** Job creation, assignment, immutable multi-supplier purchase evidence, funding classification, bounded role-scoped staff reads, web/native staff workspaces, customer collection progress, explicit customer shortage decisions and the admin/finance approved-reduction action are implemented. Add the remaining role-scoped assignment, purchase-evidence, funding/settlement and custody controls only after their exact authorization and exception boundaries are verified. Keep each order's collected goods separate and preserve historical pickup snapshots.
