...
- Both gateways remain protected by the closed storefront switch. Phase 19 replaces the legacy opening requirement without opening intake; actual operating acceptance remains separate.
...
## Implemented phase 19: warehouse-free launch readiness

- The canonical readiness report declares supplier pickup, `warehouse_required:false` and `inventory_required:false`. It retains legacy warehouse/stock observations under compatibility fields but neither can block or authorize the supplier-pickup opening path.
- Readiness requires a published merchant profile, supported tax state, no preview catalog, procurement-orderable offerings, an active supplier pickup site, active purchasing staff, an active courier, warehouse-independent available delivery capacity and healthy dependencies.
- Opening requires an administrator to confirm the exact catalog, supplier-sites, procurement, delivery, tax and operations checks and record a real reference. The successful review is separately immutable and tied to the current published policy. Existing policy publication and closed-switch safety remain unchanged.
- Renamed helpers are private even to `service_role`; only the existing admin read/write gateways remain service-executable. No client receives a direct readiness or state-changing database primitive.
- Release `b15f2d4e` publishes the gate with Supabase at 115 migrations. Exact live verification, database replay, browser, recovery and capacity gates passed. Production intake remains closed because no active pickup site, purchasing employee or courier is recorded; no test fixture was created.

## Next engineering gates — not complete

1. **Complete actual setup and acceptance.** Enter only the owner's real merchant/tax/catalog, supplier-site, purchasing/courier and delivery-capacity information, then perform the recorded six-part opening review. Automated disposable acceptance is complete; actual staff/device operating acceptance is not.
2. **Complete assigned procurement work.** The end-to-end happy path is published and disposable-tested. Keep each order's collected goods separate and preserve historical pickup snapshots while validating real operating procedures and exception ownership.
3. **Exceptions with customer consent.** The complete-removal proposal, customer decision, exact reduction and zero-purchase cancellation are implemented. Keep post-purchase cancellation closed until supplier-return, employee-liability/reimbursement and customer-refund rules are approved. Supplier cost must not silently change the agreed retail price; no cost-plus margin or purchasing fee is inferred.
4. **Delivery and finance.** Only funded, dual-confirmed collected quantities pass to courier custody. Obtain owner approval for actual supplier-payment channels/timing and employee advance/reimbursement, then implement any post-purchase return as a separate auditable action.
5. **Distribution and field acceptance.** Complete physical-device staff/customer trials and signed Apple/Google releases. Existing Expo, Android debug and iOS simulator CI are build evidence only.
