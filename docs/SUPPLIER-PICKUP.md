...
2. **Complete assigned procurement work.** Job creation, assignment, immutable multi-supplier purchase evidence, funding classification, bounded role-scoped staff reads, read-only web/native staff workspaces, customer collection progress and explicit customer shortage decisions are implemented. Build role-scoped employee/admin transaction controls only after the remaining write and exception boundaries are complete. Keep each order's collected goods separate and preserve historical pickup snapshots.
