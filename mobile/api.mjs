...
export function createApiClient({base,storage,fetchImpl=fetch,timeoutMs=20000,newKey=()=>`${Date.now()}-${Math.random().toString(36).slice(2)}-${Math.random().toString(36).slice(2)}`}={}){
 ...
 const critical=(method,path)=>method==='POST'&&(path==='/api/quotes'||path==='/api/orders'||path==='/api/shopping-lists'||path==='/api/recurring'||/\/refunds$/.test(path)||/^\/api\/substitutions\/[^/]+\/decision$/.test(path));
