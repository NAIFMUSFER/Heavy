...
import {cashReceiptFacts,orderFacts,orderLinks,procurementProgressFacts,trackingView} from './order.mjs';
...
export default function OrderDetails({order,call,onRefresh,onSupport,children,ui}){
 ...
 const [busy,setBusy]=useState(false),[tracking,setTracking]=useState(null),[code,setCode]=useState(null),[shortageNote,setShortageNote]=useState('');
 const locked=useRef(false),facts=orderFacts(order),receipt=cashReceiptFacts(order),progress=procurementProgressFacts(order),address=facts.address,slot=facts.slot,links=orderLinks(address);
 ...
 function decideShortage(decision){
  const note=shortageNote.trim(),approved=decision==='approve_removal';
  if([...note].length<3||[...note].length>1000){Alert.alert('الملاحظة مطلوبة','أدخل ملاحظة واضحة من 3 إلى 1000 حرف.');return}
  const message=approved?`سيصبح إجمالي الطلب ${money(progress.shortage.totalIfApproved)} بعد تطبيق التخفيض المنفصل، ولن تُضاف رسوم جديدة.`:'سيعود الموظف لمتابعة الجمع ولن يتغير سعر الطلب.';
  Alert.alert(approved?'الموافقة على النقص':'رفض النقص',message,[{text:'رجوع',style:'cancel'},{text:approved?'موافقة صريحة':'رفض ومتابعة الجمع',style:approved?'default':'destructive',onPress:()=>run(async()=>{await call('/api/orders/'+order.id+'/procurement-shortage-decision',{method:'POST',body:{request_id:progress.shortage.requestId,expected_revision:progress.revision,decision,note}});setShortageNote('');await onRefresh()})}]);
 }
 ...
 return <>
  ...
  {progress&&<Card><Text style={s.productTitle}>تقدم جمع الطلب</Text><Text>{progress.title}</Text><Text style={s.muted}>{progress.message}</Text>{progress.lines.map(line=><View key={line.lineId} style={s.between}><View style={{flex:1}}><Text>{line.name}{line.sizeLabel?' · '+line.sizeLabel:''}</Text><Text style={s.muted}>طُلب {line.requested} · جُمع {line.collected} · متبقٍ {line.remaining}</Text></View></View>)}{progress.shortage&&<Text>{progress.shortage.state==='pending'?'التخفيض المقترح عند الموافقة':'التخفيض المرتبط بالنقص'}: {money(progress.shortage.reduction)}</Text>}{progress.canDecide&&<><Text style={s.muted}>الموافقة تقبل حذف الكمية غير المتوفرة؛ يطبق النظام التخفيض المنفصل قبل التسليم. الرفض يعيد الطلب لمتابعة الجمع بلا تغيير السعر.</Text><TextInput style={[s.input,{minHeight:80,textAlignVertical:'top'}]} multiline maxLength={1000} value={shortageNote} onChangeText={setShortageNote} placeholder="ملاحظتك على القرار" editable={!busy}/><Btn title="الموافقة على النقص" disabled={busy} onPress={()=>decideShortage('approve_removal')}/><Btn title="رفض النقص ومتابعة الجمع" kind="outline" disabled={busy} onPress={()=>decideShortage('reject_removal')}/></>}{progress.actionRequired&&!progress.canDecide&&<Text style={s.muted}>لن يتغير طلبك قبل تسجيل موافقتك الصريحة.</Text>}<Text style={s.muted}>آخر تحديث: {when(progress.updatedAt)}</Text></Card>}
